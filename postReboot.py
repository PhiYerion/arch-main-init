...
def main():
    ...
    username = sp.check_output(
        "ls /home", shell=True, stderr=sp.STDOUT, text=True
    ).strip()
    ...
    if pmt("Games/Emulation? (Wine, proton, steam, lutris)"):
        ...
        cmd(
            "pacman -Sy ttf-liberation lib32-systemd wine wine-gecko wine-mono winetricks protontricks lib32-alsa-lib lib32-alsa-plugins lib32-libpulse lib32-pipewire pipewire-pulse lib32-libpulse flatpak;\
            pacman -Syuu"
        )
        if pmt("Nvidia?"):
    ...
    seperator = " "
    cmd("paru -S" + seperator.join(addInstall), True)
